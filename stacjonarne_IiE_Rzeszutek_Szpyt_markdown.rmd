...
```
Metoda Hellwiga wskazuje na wybranie zmiennych X4, A i B, natomiast metoda krokowa na zmienne X1, X3, X4, X6, A i B. Wybrana zostanie druga opcja 

```{r echo=TRUE, message=FALSE, warning=FALSE}
model <- lm(Y ~ X1 + X3 + X4 + X6 + A + B, data.train)
summary(model)

```
W celu sprawdzenia, które zmienne są istotne (testem t - Studenta) sprawdzimy jego założenie, jakim jest normalność reszt 
...
```
Reszty w naszym modelu nie mają rozkładu normalnego, zatem pozostawimy wszystkie zmienne, które zostały do niego wybrane. Sprawdzimy założenie MNK jakim jest stała wariancja reszt  
  
```{r echo=FALSE, message=FALSE, warning=FALSE}
plot(residuals(model), type = "l")
```
Wykres reszt wskazuje na występowanie heteroskdastyczności, ale dla pewności uruchominony zostanie test Breuscha - Pagana, którego hipotezy to:
  
  *H0: homoskedastyczność*
  
  *H1: heteroskedatyczność*
...
ncvTest(model)
```
Aby ominąć problem heteroskedastyczności użyta zostanie metoda WMNK. W tym celu swotrzono model pomocniczy, gdzie zmienną objaśnianą są reszty z poprzedniego modelu (wartość bezwzględna), a zmiennymi objaśniającymi będą zmienne z poprzedniego modelu.
```{r}
