...
```
Metoda Hellwiga wskazuje na wybranie zmiennych X4, A i B, natomiast metoda krokowa na zmienne X1, X3, X4, X6, A i B. Sprawdzimy rozkład reszt w obu tych modelach testem Shapiro - Wilka. Hipotezy:
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
model_step <- lm(Y ~ X1 + X3 + X4 + X6 + A + B, data.train)
model_hel <- lm(Y ~ X4 + A + B, data.train)
...
Żaden z tych modeli nie ma normalnego rozkładu reszt, oto ich wykresy
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
plot(density(model_step$residuals))
plot((density(model_hel$residuals)))
```

Sprawdzimy założenie o stałości wariancji składnika losowego w modelach
  
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
plot(residuals(model_step), type = "l")
plot(residuals(model_hel), type = "l")
```
Wykres reszt wskazuje na występowanie heteroskdastyczności, ale dla pewności uruchomiony zostanie test Breuscha - Pagana, którego hipotezy to:
...
Aby ominąć problem heteroskedastyczności użyta zostanie ważona metoda największej wiarygodności. W tym celu stworzymy model pomocniczy, gdzie zmienną objaśnianą są reszty z poprzedniego modelu (wartość bezwzględna), a zmiennymi objaśniającymi będą zmienne z poprzedniego modelu.
```{r echo=TRUE, message=FALSE, warning=FALSE}
...
```{r}
wages <- (model.helpmegod$fitted.values)^-2
#model pomocniczny - zmienna objaśniana to reszty do kwadratu a zm objaśniające to zmienne z poprzedniego modelu

model.wg <- lm(Y ~ X1 + X3 + X4 + X6 + A + B, data.train, weights=wages)
summary(model.wg)
```
